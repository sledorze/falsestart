...
| `--rules pkg:<name>` | Rules from an installed package, e.g. `pkg:@acme/falsestart-rules`, optionally with a subdirectory. `--rules` is single-valued: given twice, or alongside the directory form, it is REFUSED with exit 1 rather than ranked.                                                                                                                                                                                          |
...
| `--path <p>`         | A real path to probe, with `--doctor`. Repeatable. Naming one ASSERTS it should be in scope: `--doctor` exits 1 when no rule applies to it. The built-in probes stay non-fatal. Refused in every other mode.                                                                                                                                                                                                         |
...
`language: tsx`, the parser is picked by that rather than by the extension, and all six do fire on
