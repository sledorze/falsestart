...
- bd10eec: `--doctor` now fails when no rule loaded from any source, and two shipped documents that said
...
- da5470a: `--doctor --path <p>` probes a path you actually have, and fails when no rule applies to it.
...
- bd10eec: A config override naming a rule this invocation did not load is now **reported by `--doctor`**
...
- bd10eec: A leading `!` in `files` or `ignores` is now refused at load, in rule documents and config overrides
  alike.
...
- bd10eec: Naming `--rules` twice, or `--preset` twice, is now **refused** instead of ranked.
...
- db8f9eb: Two inputs that made the guard go silent are now refused or handled, and a defect can no longer exit
...
- fd2d5f5: `docs/reference.md` now documents the seven exports it was missing — six of them added the same week,
...
- cf3648f: `--help` no longer claims `--warn-unscoped` fires on every `.js` write. It does not, and has not
...
- 9d93662: Document that a rule can match a comment, because an adopter concluded it could not.
