...
Sourcing a **leading word** from a named practitioner's own coined vocabulary rather than inventing one — Parnas' _secret_, Dijkstra's _separation of concerns_, Deming's _the system, not the person_. Worked by thinking out loud: name the concern, ask which practitioner owns it, search to confirm the term and its exact wording, then harvest it. A grounded word inherits both precision (a practitioner already sharpened it against a real failure mode) and pretraining priors no coined word carries for free.

_Avoid_: sourcing, borrowing, citation
